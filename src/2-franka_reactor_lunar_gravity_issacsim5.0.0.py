"""
Franka Robot - Move Nuclear Reactor from Left to Right (with RMPflow)
======================================================================
A standalone script using RMPflow for smooth inverse kinematics control.
The robot will smoothly move a nuclear reactor from its left side to right side.

? MOON GRAVITY VERSION - Gravity set to 1.62 m/s² (1/6th of Earth)

Compatible with Isaac Sim 5.0.0 - PRODUCTION READY WITH PROPER IK

Usage:
Linux: ~/isaacsim/python.sh franka_reactor_moon.py
Windows: C:\isaacsim\python.bat franka_reactor_moon.py
"""
...
class FrankaReactorMoverRMPflow:
    """
    State machine controller using RMPflow for smooth motion planning.
    Handles nuclear reactor transport with safe gripper control.
    """
    ...
    def __init__(self, world: World, franka: Franka):
        ...
        # End effector positions for reactor transport
    ...
    def setup_scene(self):
        ...
        # Add nuclear reactor on the left side (blue cylinder - Cherenkov radiation color)
        self.reactor = self.world.scene.add(
            DynamicCylinder(
                ...
                radius=0.036,
                height=0.08,
                color=np.array([0.0, 0.5, 1.0]),  # Blue - Cherenkov radiation blue
                mass=0.05
            )
        )
        ...
        print(f" - Nuclear reactor (left): {self.reactor_pickup_pos}")
        print(f" - Target position (right): {self.reactor_place_pos}")
...
class SimulationController:
    ...
    def __init__(self, world: World, controller: FrankaReactorMoverRMPflow):
        self.world = world
    ...
    def setup_gui(self):
        ...
        try:
            ...
            with dpg.window(label="Franka Simulation Control", tag="main_window",
                           width=400, height=400, pos=(50, 50), no_close=True):
                dpg.add_text("Franka Nuclear Reactor Mover - RMPflow ?", color=(100, 200, 255))
                dpg.add_text("Moon Gravity Mode ?", color=(200, 200, 100))
                dpg.add_separator()
                ...
                # Gravity info
                dpg.add_text("Gravity: 1.62 m/s² (Moon)", tag="gravity_text", color=(150, 200, 255))
            ...
            dpg.create_viewport(title="Franka Control Panel ?", width=420, height=450, vsync=False)
            dpg.setup_dearpygui()
            dpg.show_viewport()
...
def main():
    """Main function"""
    print("\n" + "="*60)
    print("FRANKA NUCLEAR REACTOR MOVER WITH RMPFLOW")
    print("Smooth inverse kinematics control")
    print("? MOON GRAVITY MODE - 1.62 m/s²")
    print("Isaac Sim 5.0.0")
    print("="*60)

    # Create world with moon gravity
    world = World(stage_units_in_meters=1.0)

    # Set moon gravity (1/6th of Earth's gravity)
    # Moon gravity = 1.62 m/s²
    print("\n? Setting moon gravity...")
    world.get_physics_context().set_gravity(value=-1.62)  # Negative because it's downward
    print(f"? Gravity set to 1.62 m/s² (Moon gravity)")
    ...
    # Create controller
    controller = FrankaReactorMoverRMPflow(world, franka)
    ...
    print("? Moon gravity active - expect slower falling motion!")
    print("? Nuclear reactor transport operations standing by")
    print("="*60 + "\n")
        ...
        try:
            ...
            if sim_controller.is_running and world.is_playing() and not sim_controller.restart_requested:
                ...
                if done and not sim_controller.is_task_complete:
                    ...
                    print("Nuclear reactor successfully transported from left to right")
                    print("? Moon gravity effects observed during motion")
                    print("? Reactor repositioning complete - all systems nominal")
                    print("="*60)
